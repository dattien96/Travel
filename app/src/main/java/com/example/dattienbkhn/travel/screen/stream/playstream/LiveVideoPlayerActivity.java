...
public class LiveVideoPlayerActivity extends AppCompatActivity implements OnClickListener, ExoPlayer.EventListener,
        PlaybackControlView.VisibilityListener, PlayStreamContract {
    public static final String LIVE_PLAYER_ACT_FLAG = "LivePlayerActivity";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        bd = getIntent().getExtras().getBundle(LIVE_PLAYER_ACT_FLAG);
        mCurAccId = bd.getInt("accId", -1);
        streamType = bd.getInt("streamType");
        if (streamType == Constant.STREAM_TYPE_LIVE_RTMP) {
            isStream.set(true);
        }


        if (SocketConfig.getSocketInstance() != null) {
            SocketConfig.getSocketInstance().on("new_stream", onNewMessageInlayer);
        }
    ...
    public void play(View view) {
        if (streamType == Constant.STREAM_TYPE_VOD_RTMP) {
            //Toast.makeText(this, "Play Vod RTMP", Toast.LENGTH_SHORT).show();
            String URL = "";
            String vodName = bd.getString("vodName", "");
            URL = Constant.VOD_BASE_URL + vodName;
            initializePlayer(URL);
            videoStartControlLayout.setVisibility(View.GONE);
        } else {
            AppRepository.getAccountRepo().getStreamState(mCurAccId)
                        ...
                        public void accept(WrapperResponse<Boolean> booleanWrapperResponse) throws Exception {
                            if (booleanWrapperResponse.getMessage().equalsIgnoreCase(ResponseCode.StreamState.toString())) {
                                ...
                                if (booleanWrapperResponse.getData()) {
                                    //Toast.makeText(LiveVideoPlayerActivity.this, "Dang live stream", Toast.LENGTH_SHORT).show();
                                    String URL = "";
                                    Date date = new Date();
                                    Calendar cal = Calendar.getInstance();
                                    cal.setTime(date);
                                    URL = Constant.RTMP_BASE_URL + mCurAccId + "-" + cal.get(Calendar.DAY_OF_MONTH) +
                                            "-" + (cal.get(Calendar.MONTH)+1) + "-" + cal.get(Calendar.YEAR);
                                    initializePlayer(URL);
                                    videoStartControlLayout.setVisibility(View.GONE);
                                } else {
                                    Toast.makeText(LiveVideoPlayerActivity.this, "Ket thuc stream", Toast.LENGTH_SHORT).show();
                                    showConfirmDialog();
                                }
                            }
                        }
    ...
    public void showConfirmDialog() {
        Toast.makeText(this, "Stream Stoped", Toast.LENGTH_SHORT).show();
        if (SocketConfig.getSocketInstance() != null) {
            SocketConfig.getSocketInstance().off("new_stream", onNewMessageInlayer);
        }
        finish();
    }
    ...
    private Emitter.Listener onNewMessageInlayer = new Emitter.Listener() {
